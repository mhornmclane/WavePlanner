...
In both replays, a diamond marks overlapping runners: the next runner departed before the incoming runner arrived. A circle marks no overlap, even when arrival is after the scheduled release. Start and Finish remain circles. Inspection gives the release, time late, and whether the next runner actually departed before arrival (respecting gate delays). Each team has one marker. Changing years pauses playback while retaining the exchange; leaving the section pauses and preserves playback preferences.
